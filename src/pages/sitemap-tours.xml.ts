...
export const GET: APIRoute = async () => {
  ...
  if (d1Db) {
    try {
      const res = await d1Db
        .prepare("SELECT slug, updatedAt, featuredImage FROM Tour WHERE status = 'published' OR status = 'publish'")
        .all();
      items = res.results || [];
    } catch (e) {
  ...
  for (const item of items) {
    ...
    xml += `  <url>\n    <loc>https://thericetour.com/tour/${item.slug}</loc>\n`;
    if (item.updatedAt) {
      try {
    ...
    if (item.featuredImage) {
      const imgUrl = getCanonicalMediaUrl(String(item.featuredImage));
