...
const LADAKH_PHOTOS: PhotoItem[] = [
  {

    id: 'glacier-calve',
    title: 'Sông Băng Ladakh',
    ...
    narrative: 'Khung cảnh kỳ vĩ khi những dòng sông băng khổng lồ nối liền với dãy núi tuyết hùng vĩ, tạo nên một tuyệt tác thiên nhiên chỉ có ở Ladakh.',
    category: 'cryo'
  },
  {
    id: 'cruise-glacier',
...
export default function PhotoGallery() {
  const [activePhoto, setActivePhoto] = useState<PhotoItem>(LADAKH_PHOTOS[0]);
  ...
  const nextPhoto = () => {
    const idx = LADAKH_PHOTOS.findIndex(p => p.id === activePhoto.id);
    const nextIdx = (idx + 1) % LADAKH_PHOTOS.length;
    setActivePhoto(LADAKH_PHOTOS[nextIdx]);
  };

  const prevPhoto = () => {
    const idx = LADAKH_PHOTOS.findIndex(p => p.id === activePhoto.id);
    const prevIdx = (idx - 1 + LADAKH_PHOTOS.length) % LADAKH_PHOTOS.length;
    setActivePhoto(LADAKH_PHOTOS[prevIdx]);
  };

  return (
    <div id="ladakh-sensory-gallery" className="bg-[#10191B]/95 rounded-3xl border border-white/10 p-6 md:p-8 text-white">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between pb-6 border-b border-white/10 gap-4 text-left">
        <div>
          ...
          <p className="font-serif italic text-sm md:text-xs text-stone-450">
            Khám phá ống kính chân thật tôn sùng sự hoang dã và hoành vĩ vô biên của Ladakh.
          </p>
        </div>
