...
import { PART1_CONFIG, ALASKA_HIGHLIGHTS, ALASKA_TIMELINE } from './data/alaskaData';
...
export default function App({ featuredImage, config = PART1_CONFIG }: AppProps) {
  // Reading mode settings state
  const [theme, setTheme] = useState<ReadingTheme>('midnight-spruce');
  ...
  const [selectedHighlight, setSelectedHighlight] = useState<GeographicHighlight>(ALASKA_HIGHLIGHTS[1]);
      ...
      <section className="max-w-4xl mx-auto px-6 pt-12 pb-6 flex flex-col items-center justify-center space-y-4">
        ...
        <p className="font-serif italic text-sm md:text-xs text-stone-400 text-center max-w-lg leading-relaxed">
          {viewStyle === 'magazine'
            ...
            : 'Đậm đặc phong vị Alaska: Trang sách gáy xoắn mộc bọc sương đọng, tọa độ thời tiết thực địa & sớ triện đỏ dán tên lữ hành.'}
        </p>
      </section>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                    ...
                    <div className="lg:col-span-8 space-y-5">
                      {sec.paragraphs.map((pText, pIdx) => {
                        ...
                        const isImageOnly = pText.trim().startsWith('<figure');

                        if (isImageOnly) {
                          return <div key={pIdx} className="w-full my-4 md:my-8 clear-both" dangerouslySetInnerHTML={{ __html: pText }} />;
                        }
                          ...
                          >
                            {isFirstP ? (
                              <>
                                ...
                                <span dangerouslySetInnerHTML={{ __html: pText.slice(1) }} />
                              </>
                            ) : (
                              <span dangerouslySetInnerHTML={{ __html: pText }} />
                            )}
                          </p>
                        );
                      })}
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start relative z-10">
                    ...
                    <div className="lg:col-span-8 space-y-4">
                      {sec.paragraphs.map((para, index) => {
                        const isImageOnly = para.trim().startsWith('<figure');

                        if (isImageOnly) {
                          return <div key={index} className="w-full my-4 md:my-8 clear-both" dangerouslySetInnerHTML={{ __html: para }} />;
                        }
