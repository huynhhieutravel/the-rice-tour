...
const INITIAL_MESSAGES: GuestbookMessage[] = [
  {
    ...
    message: '80 chuyến đi là một con số biết nói. Từng viễn du Ladakh với FIT Tour vào năm ngoái, tôi hoàn toàn ấn tượng với sự chuẩn bị chu đáo và nồi phở bò nóng hổi của các bạn giữa thung lũng lạnh giá. Chúc FIT Tour ngày càng phát triển!',
  ...
  },
  {
    ...
    message: 'Khâm phục sức khỏe và sự tận tâm của đội ngũ guide bản xứ. Nhờ các cháu đệm sưởi điện và trà gừng mật ong mà cô cùng bác trai U70 vẫn đi về khỏe re, ngày nào cũng nhắc lại kỷ niệm ngắm sao ở hồ Pangong Tso.',
    createdAt: '05/06/2026, 17:15',
    location: 'Đà Nẵng'
  },
  {
    id: 'm-3',
    name: 'Minh Quân (Hành trình Zanskar)',
    message: 'Hành trình Motor Trip băng qua thung lũng Zanskar thực sự là một trải nghiệm để đời. Sự can trường và am hiểu địa hình của các tài xế FIT Tour đã giúp tôi an tâm tuyệt đối vượt qua mọi cung đường sỏi đá.',
...
export default function Guestbook() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('ladakh_80_guestbook');
    if (saved) {
      try {
      ...
      setMessages(INITIAL_MESSAGES);
      localStorage.setItem('ladakh_80_guestbook', JSON.stringify(INITIAL_MESSAGES));
    }
  }, []);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setMessages(updated);
    localStorage.setItem('ladakh_80_guestbook', JSON.stringify(updated));
